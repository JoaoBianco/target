...
function calcularFaturamento(dados) {
  const diasValidos = dados.filter(item => item.valor > 0)

  const valores = diasValidos.map(item => item.valor)
  ...
  const diasAcimaDaMedia = diasValidos.filter(item => item.valor > mediaMensal).length
